...
### Added
- Custom redaction rules via a project config file (closes the config-driven part of #2). A `secure-log2test.toml` in the working directory may carry a `[redaction]` table with `extra_header_names` (exact header names) and `extra_field_patterns` (regexes matched as a substring against header, body-field, and URL-parameter names, the same surface the built-in matcher uses). The built-in blacklist always stays on; config only adds. Rules load and install at CLI startup through a new `core/redaction_config.py`; a pattern that does not compile fails fast with a clear `ValueError` naming the offending regex. Config is parsed with the standard-library `tomllib` on Python 3.11+ and `tomli` on 3.10. 13 tests in `tests/test_redaction_config.py`. JSON-path body-field redaction stays open on #2.
- Optional response-body assertions in the generated pytest suite (closes #1). A new `--assert-config <file.json>` flag points at a JSON config whose `rules` target an endpoint by `method` + `url` and declare `expect_fields` (top-level JSON field equality) and/or a `schema` (a JSON Schema file). Log exports rarely carry the response body, so the expectations are user-declared rather than scraped from the logs. A referenced schema is read at generation time and inlined into the generated test, so the emitted suite stays self-contained and validates with `jsonschema` (an optional dependency the generated test imports only when a schema rule is used). Each generated request guards on the response `content-type` so a non-JSON response skips the body check. Config loading, endpoint matching, and the emitted source live in a new `core/assertions.py`; 12 tests in `tests/test_assertions.py`. Applies to `--format pytest` only.
...
- Sample Splunk exports `data/sample_splunk_export.csv` and `data/sample_splunk_export.json`, plus 19 tests in `tests/test_splunk_parser.py` (CSV/JSON parsing, alias fields, URL/header/body redaction, skip of non-HTTP events, source auto-detection, CLI integration, and robustness: a single corrupt NDJSON line is skipped and counted, fractional and unit-suffixed durations/statuses are coerced instead of dropped, and CSV content without a `.csv` suffix is still detected). The suite is now 127 tests.
